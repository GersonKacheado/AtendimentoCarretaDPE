...
<div style="position: relative;">
    <div class="panel panel-success" id='profissionais'>
        ...
        <div class='p-0 panel-body col-md-12 btn-group' role='group' aria-label='Exemplo'>
            <?php if(isset($_GET['resp']) && $_GET['resp']=='Tarcisoedit'){ @$idfila = $_GET['idfila']; @$nome = $_GET['nome']; @$idservico = $_GET['idservico']; ?>
                <form class="p-0" method="POST" action="model/controller/editar_dados.php" >
                    ...
                    <div class="form-group">
                        <div class="col-md-10 dropdown">
                            ...
                            <input  class="form-control border-0" type="name" name='nome' value="<?php echo $nome; ?>"  readonly>
                        </div>   
 ...
 <!-- INÍCIO Confirmação dos serviços -->                       
                        
                        <div class="col-md-1 dropdown" style="display: block;">
                            <label for="btnDadosSERVICOS">Ação</label><br>
                            <?php 
                                if(isset($_GET['mudarservico']) && $_GET['mudarservico'] == 'mudarservico')
                                {
                                    echo "<button type='submit' class='btn btn-primary' name='btnEditarSERVICOS'>
                                    <i style='font-size:24px' class='fa fa-edit text-white 3x'></i> Editar Serviço</button>";

                                }else{
                                    echo "<button type='submit' class='btn btn-primary'  name='btnDadosSERVICOS'>Confirmar</button>";
                                }
...
<!-- FIM Confirmação dos serviços -->                       
 

                        <div class="mt-10 col-md-12 btn-group table-responsive">
                            <h3 class="ps-12 panel-titulo" style="color:green">Atendimento Relaizados</h3>
                            <table class="col-md-12 table table-striped table-responsive" id="minhaTabelaE">
                                <thead>
                                    <tr class="h5">
                                        <th scope="col">#</th>    
                                        <th scope="col">NOME DO ASSISTIDO</th><th scope="col">NÚCLEO</th> <th scope="col">AÇÃO</th> <th scope="col">ATENDENTE</th> <th scope="col">OBSERVAÇÃO</th><th scope="col">T. INÍCIO</th><th scope="col">T. FIM</th>
                                        <th scope="col">TIPO DOCUMENTO</th><th scope="col">REGISTRO</th><th scope="col">EDIÇÃO</th>
                                    </tr>
                                </thead>
                                <tbody style="color:#000">
                                    
                                    <?php
                                        $selecionar_dados->setListaViewAssistidoAtendimentos(@$idfila);
                                    ?>                    
...
<!--
                        <div class="col-md-12 dropdown">
                            <label for="nucleo_atendimento">Núcleo:</label>
                            <div class="d-flex"> 
                            <?php // $selecionar_dados->setNucleoAtendimentoInput(); ?>
                            </div>
                            
                            <label for="nucleo_atendimento">Ação:</label>
                            <div class="d-flex"> 
                                <?php // $selecionar_dados->setAcaoAtendimentoInput(); ?>
                            </div>
                        </div> 
-->                     
                        ...
                        <div class="col-md-1 dropdown" style="display: block;">
                            ...
                            <button type="submit" class="btn btn-primary"  name='btnDadosEdit'>Finalizar Atendimento</button>
                        </div>
            ...
            <?php }else{ ?>
                ...
                <?php 
                    //$nucleo     = fopen('public/select_text/nucleo.txt', 'r');
                    //$acao       = fopen('public/select_text/acao.txt', 'r');
                ?>
                ...
                <form class="p-0" method="POST" action="model/controller/inserir_dados.php" >
                    <div class="form-group">
                        ...
                        <div class="col-md-9 dropdown">
                            <label for="nucleo_atendimento">Observações</label>
                            <textarea class='form-control' name='observacao' id='observacao' placeholder="Observações, se necessário..."></textarea>                        
                        </div> 
...
//SELECT COUNT(*) as total_servicos, f.nome FROM `tbl_fila` f, tbl_servicos s WHERE f.fk_evento = 18 AND s.fk_idfila = f.idfila GROUP by f.nome; 
...
<script>
    $("#obsnucleos").blur(_ => {
        if($('#obsnucleos :selected').text()=='EMISSÃO RG')
            $("#atendente").val("4");
            $(".atendente").val("4");
            $("#obsacoes").val("64");
            $('#observacao').focus();
    ...
    $("#obsacoes").blur(_ => {
        if($('#obsacoes :selected').text()=='EMISSÃO RG')
            $("#atendente").val("4");         
    });
</script>
