...
*/
    ...
    function setAtendenteAtendimento(){
        ...
        $sql = "SELECT * FROM `tbl_user` u, tbl_historico h where u.habilitado = 'S' and u.fk_tipo_user = 3 and h.fk_iduser = u.iduser and h.fk_idevento = '".$id_evento."'  ORDER BY `u`.`nome_user` ASC";
...
// FIM evento que deseja trabalhar
    ...
    function setAtendenteSolucaoEdit(){ 
        ...
        self::execute($sql);
        $resultado = $this->stmt->fetch(PDO::FETCH_OBJ);
        echo "<textarea type='text' class='form-control' name='solucao' id='solucao'>".@$resultado->solucao."</textarea>";
    }
...
function setListaAtendimentoFinalizadosPorAssistido(&$contador, &$valores, $id){
    ...
    $sql = "SELECT * FROM `VIEW_RELATORIO_CARRETA` WHERE cpf like  '".$id."' or nome like  '".$id."' ORDER BY nome";
...
// Ações
    ...
    function setAcaoGrafico(&$indiceacao, &$valoracao, &$acao, $id_evento){
        ...
        while($resultado = $this->stmt->fetch(PDO::FETCH_OBJ)) {
            ...
            $acao[$indiceacao]      = ($resultado->dsc_acao=='?')?'Desistencia s/ Atendimento':$resultado->dsc_acao;
            $indiceacao++;
        }        
    ...
    function setAcaoGraficoPrevio(&$indiceacao, &$valoracao, &$acao, $id_evento){
        ...
        while($resultado = $this->stmt->fetch(PDO::FETCH_OBJ)) {
            $valoracao[$indiceacao] = $resultado->total;
            $acao[$indiceacao]      = $resultado->dsc_acao;
            $indiceacao++;
        }        
...
// Núcleos
    ...
    function setNucleoGrafico(&$indicenucleo, &$valornucleo, &$nucleo, $id_evento){
        ...
        while($resultado = $this->stmt->fetch(PDO::FETCH_OBJ)) {
            ...
            $nucleo[$indicenucleo]      = ($resultado->dsc_nucleo=='?')?'Desistencia s/ Atendimento':$resultado->dsc_nucleo;
            $indicenucleo++;
        }        
...
// Atendimentos por membros
    ...
    function setDefGrafico(&$indicedef, &$valordef, &$def, $id_evento){
        ...
        while($resultado = $this->stmt->fetch(PDO::FETCH_OBJ)) {
            ...
            $def[$indicedef]      = $resultado->nome_user;
            $indicedef++;
        }        
...
// Atendimentos por Tipo
    ...
    function setAmGrafico($id_evento){
        $sql = "SELECT f.nome_user, f.nome, f.cpf, f.telefone, f.endereco, f.tempo_de_atendimento, f.fk_funcao, f.tempoinicio, f.tempofim, COUNT(*) as total 
        ...
        while($resultado = $this->stmt->fetch(PDO::FETCH_OBJ)) {
            ...
            echo "<tr>
                ...
                <td>".$resultado->tempo_de_atendimento."</td></tr>";
            

        }
        @$html .= "<tr><td class='h3 bg-success' colspan='2'>Total de Assistdos Atendidos: </td><td colspan='3' class='h3 bg-success'>".@$total."</td></tr>";
        echo $html;
...
// INICIO Pesquisa de Usuario sistema - sessao administrador   - GERAL
    ...
    function setNucleoAtendimentoInput(){
        ...
        self::execute($sql);
        while($resultado = $this->stmt->fetch(PDO::FETCH_OBJ)) {
            echo "<input type='checkbox' name='".$resultado->dsc_nucleo."'> <span style='padding:0 42px 0 0'>".$resultado->dsc_nucleo."</span>";  
        }
    }
    ...
    function setAcaoAtendimentoInput(){
        ...
        self::execute($sql);
        while($resultado = $this->stmt->fetch(PDO::FETCH_OBJ)) {
            echo "<input type='checkbox' name='".$resultado->dsc_acao."'> <span style='padding:0 42px 0 0'>".$resultado->dsc_acao."</span>";        
        }
    }
...
/*
SELECT id, data_entrada, data_saida, cpf, nome, endereco, telefone, observacao, solucao, idade, acao_prolongada, agua_tratada, area_buscou_nao_houve_atendimento,
avaliacao_defensor, avaliacao_tempo, condicao_imovel, conhecimento_acao, cor, data_entrevista, escolaridade, estado_civil, esta_gestante, fossa_septica, frequencia_de_visita, importancia_carreta,
local_residencia, melhorar_atendimento, ocupacao, possui_filhos_qtd, providencia_tomada, rede_esgoto, rede_internet, renda_domiciliar, tipo_residencia
